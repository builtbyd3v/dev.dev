...
pub(crate) fn initialize_app(
    ...
    // Embedded MCP server exposing browser-pane control tools (navigate/click/type/screenshot/
    // evaluate/console) to Claude Code CLI and other MCP clients. Kill switch:
    // `browser_mcp::BROWSER_MCP_ENABLED`.
    #[cfg(not(target_family = "wasm"))]
    if browser_mcp::BROWSER_MCP_ENABLED {
        ctx.add_singleton_model(browser_mcp::AppBrowserBridge::new);
        ctx.add_singleton_model(browser_mcp::BrowserMcpServer::new);
    }
    #[cfg(feature = "local_fs")]
    if matches!(
        launch_mode,
