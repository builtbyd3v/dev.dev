...
pub enum WorkspaceAction {
    ...
    OpenBrowserPane {
        url: Option<String>,
        /// Terminal session that triggered this open (dev-server auto-detect only). Recorded so
        /// attach-to-terminal actions can target the session that actually started the dev
        /// server instead of whatever tab happens to be active later. `None` for
        /// user-initiated opens (globe button, port-watcher) which have no single owning
        /// session.
        source_session_id: Option<TerminalSessionId>,
    },
    /// Toggles a browser pane in the active tab: closes it if one's visible, otherwise opens one
    /// (reusing the last-visited URL if any). Used by the sidebar globe button and Ctrl+Shift+B.
    /// Distinct from `OpenBrowserPane`, which is open-or-focus (never closes) and is what
    /// dev-server auto-detection uses so HMR reloads don't close the pane out from under it.
    ToggleBrowserPane,
    /// Internal bookkeeping: records the URL a browser pane was showing when it closed, so a
    /// later toggle/open picks up where the user left off. Not user-facing, dispatched by
    /// `BrowserPane::detach`.
    StashLastBrowserPaneUrl(String),
    /// Attaches a browser pane's screenshot (+ current URL) to the active conversation's pending
    /// AI context, same as pasting an image. Dispatched by `BrowserView`'s "Attach to AI" header
    /// button after it captures the page via CDP.
    AttachBrowserScreenshot {
        png_bytes: Vec<u8>,
        url: String,
    },
    /// Attaches a picked DOM element (outerHTML + computed selector) to the active conversation's
    /// pending AI context. Dispatched by `BrowserView` after the init script's element selector
    /// reports a click (see `WebViewHost::set_on_element_selected`).
    AttachBrowserElement {
        html: String,
        selector: String,
        url: String,
    },
    /// Attaches recently captured console errors (`WebViewHost::console_errors_text`) to the
    /// active conversation's pending AI context. Dispatched by `BrowserView`'s "Attach console"
    /// toolbar button.
