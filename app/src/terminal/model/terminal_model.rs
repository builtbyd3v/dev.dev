...
lazy_static::lazy_static! {
    /// Matches a dev-server URL (e.g. `http://localhost:5173/foo`) in freshly-flushed PTY
    /// output. Captures the full URL including path/query so the browser pane opens to the
    /// exact page the dev server printed, not just the origin.
    static ref DEV_SERVER_URL_REGEX: regex::Regex =
        regex::Regex::new(r"https?://(localhost|127\.0\.0\.1|0\.0\.0\.0):\d+\S*").unwrap();
}
...
pub struct TerminalModel {
    ...
    /// Dev-server URLs already emitted as [`Event::DevServerUrlDetected`] per block, so HMR
    /// reprints of the same URL don't re-open a browser pane. Cleared whenever the active block
    /// changes (see `on_finish_byte_processing`).
    dev_server_url_notifications: HashMap<BlockId, HashSet<String>>,
}
...
impl TerminalModel {
    ...
    fn new_internal(
        ...
        Self {
            ...
            dev_server_url_notifications: HashMap::new(),
        }
    }
    ...
    /// Scans the tail of the active grid for a dev-server URL (e.g. one printed by
    /// Vite/webpack-dev-server/Next.js on startup or HMR reload) and emits
    /// [Event::DevServerUrlDetected] the first time a given URL is seen for the active block.
    /// Debounced per-block so HMR reprints of the same URL don't repeatedly fire; the debounce
    /// set is dropped whenever the active block changes.
    ///
    /// Scans the RENDERED grid rather than the raw PTY bytes: TUI apps (e.g. Claude Code's CLI)
    /// paint with cursor-positioned writes, so a URL can arrive fragmented or out of visual
    /// order in the byte stream even after ANSI stripping. Post-parse grid lines are the one
    /// place the text is guaranteed contiguous and in reading order.
    // ponytail: setting gate goes here, e.g. `if !AutoOpenDevServerBrowserPaneSetting.value() { return; }` -
    // skipped for MVP, hardcoded enabled.
    fn check_for_dev_server_url(&mut self) {
        // Perf cap: a freshly printed URL is by definition near the bottom of the grid, so only
        // the last few visible-screen-fulls of rows need rescanning per byte batch.
        const MAX_SCANNED_ROWS: usize = 50;

        let block_id = self.active_block_id().clone();
        self.dev_server_url_notifications
            .retain(|id, _| *id == block_id);
        ...
        if total_rows == 0 || columns == 0 {
            return;
        }
        let start_row = total_rows.saturating_sub(MAX_SCANNED_ROWS);
        let text = grid.bounds_to_string(
            Point::new(start_row, 0),
            Point::new(total_rows - 1, columns - 1),
            /* include_esc_sequences */ false,
            RespectObfuscatedSecrets::No,
            /* force_secrets_obfuscated */ false,
            RespectDisplayedOutput::No,
        );

        // Take the LAST match in the scanned region, not the first: a fallback message like
        // Next.js's "⚠ Port 3000 is in use, using 3001" followed by "Local: http://localhost:3001"
        // would otherwise lock onto the stale/never-listening port if we stopped at the first hit.
        // Also skip any match whose line reports the port as unavailable, so a URL mentioned only
        // in passing ("Port 3000 is in use") is never mistaken for the server's real address.
        let found = DEV_SERVER_URL_REGEX
            .find_iter(&text)
            .collect::<Vec<_>>()
            .into_iter()
            .rev()
            .find(|found| {
                ...
                !line.contains("in use") && !line.contains("eaddrinuse")
            });
        ...
        let seen_urls = self
            .dev_server_url_notifications
            .entry(block_id.clone())
            .or_default();
        if seen_urls.insert(url.clone()) {
            self.event_proxy
                .send_terminal_event(Event::DevServerUrlDetected { block_id, url });
        }
    }
}
...
impl ansi::Handler for TerminalModel {
    ...
    fn on_finish_byte_processing(&mut self, input: &ansi::ProcessorInput<'_>) {
        ...
        // The grid has already been updated with this batch's bytes by the time this hook runs,
        // so the rendered lines the batch touched are scannable here.
        self.check_for_dev_server_url();
