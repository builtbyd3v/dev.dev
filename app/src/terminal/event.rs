...
pub enum Event {
    ...
    /// Sent when a dev-server URL (e.g. `http://localhost:5173`) is detected in freshly-flushed
    /// PTY output. See comment above [crate::terminal::ModelEvent::DevServerUrlDetected].
...
impl Debug for Event {
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Event::DevServerUrlDetected { url, .. } => write!(f, "DevServerUrlDetected({url})"),
