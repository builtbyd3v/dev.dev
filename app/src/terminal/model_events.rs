...
impl ModelEventDispatcher {
    ...
    fn handle_terminal_model_event(&mut self, event: Event, ctx: &mut ModelContext<Self>) {
        let event_to_emit = match event {
            ...
            Event::DevServerUrlDetected { block_id, url } => {
                ModelEvent::DevServerUrlDetected { block_id, url }
            }
...
pub enum ModelEvent {
    ...
    /// Emitted when [Event::DevServerUrlDetected] fires for the active block. Carries `block_id`
    /// so the view layer can be extended to scope handling to a specific block if needed (unused
    /// for now - view handler ignores it and dispatches unconditionally).
    DevServerUrlDetected {
        block_id: BlockId,
        url: String,
    },
