    ...
    fn restore_pane_leaf(
        ...
        let result = match leaf.contents {
            ...
            LeafContents::Browser => {
                // Browser panes hold a native WebView2 child HWND that can't be restored;
                // they're opened on-demand via workspace actions.
                Err(anyhow::anyhow!("Browser panes are not restored"))
            }
        };
