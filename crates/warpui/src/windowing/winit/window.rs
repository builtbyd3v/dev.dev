...
impl crate::platform::Window for Window {
    ...
    fn raw_window_handle(&self) -> Option<raw_window_handle::RawWindowHandle> {
        ...
        // winit re-exports rwh 0.6, same crate instance as warpui_core's `raw-window-handle`
        // dep (verified via Cargo.lock), so `RawWindowHandle` unifies without conversion.
        self.inner
            .borrow()
            .as_ref()
