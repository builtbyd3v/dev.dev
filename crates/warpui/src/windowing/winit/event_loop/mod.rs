...
impl EventLoop {
    ...
    fn redraw_window(
        ...
        let render_result = (|| {
            ...
            let new_scene = if !window.has_scene() {
                let scene = self.callbacks.for_window(window).build_scene(window);
                // A scene build is this window's one complete paint pass, so any webview host
    ...
    /// Returns Win32 keyboard focus to `window_id`'s own HWND if something else (typically a
    /// WebView2 browser-pane child) holds it. `GetFocus` returns null when focus belongs to
    /// another process's window — WebView2's child HWNDs live in msedgewebview2.exe — so
    /// "focus != our HWND" covers both the child-focused and the focus-lost cases, and the
    /// `SetFocus` is skipped entirely in the common case where we already have focus.
    #[cfg(target_os = "windows")]
    fn reclaim_native_focus(&mut self, window_id: winit::window::WindowId) {
        use windows::Win32::Foundation::HWND;
        use windows::Win32::UI::Input::KeyboardAndMouse::{GetFocus, SetFocus};

        let Some(app_window_id) = self
            .state
            .windows
            .get(&window_id)
            .map(|state| state.window_id)
        ...
        let Some(window) = self
            .ui_app
            .read(|ctx| ctx.windows().platform_window(app_window_id))
        else {
            return;
        };
        let Some(winit::raw_window_handle::RawWindowHandle::Win32(handle)) =
            window.raw_window_handle()
        else {
            return;
        };
        let own_hwnd = HWND(handle.hwnd.get() as _);
        if unsafe { GetFocus() } != own_hwnd {
            let _ = unsafe { SetFocus(Some(own_hwnd)) };
        }
    }

    /// Handles a [`winit::event::WindowEvent`].
    fn handle_window_event(&mut self, window_id: winit::window::WindowId, evt: WindowEvent) {
        // A native child window (the WebView2 browser pane) can hold Win32 keyboard focus while
        // this top-level window stays active: winit still receives WM_MOUSE* for clicks on its
        // own surface, but nothing ever calls SetFocus, so typing would keep going to the child.
        // Reclaim focus on any mouse-down over our surface. Clicks inside the child HWND never
        // reach winit, so this can't fight the webview for focus.
        #[cfg(target_os = "windows")]
        if matches!(
            evt,
            ...
            self.reclaim_native_focus(window_id);
        }
