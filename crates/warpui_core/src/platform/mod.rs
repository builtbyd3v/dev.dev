...
pub trait Window: 'static + WindowContext + std::any::Any {
    ...
    /// Returns the raw OS window handle backing this window, if available. Used by embedders
    /// (e.g. a WebView2 host) that need the native HWND/NSView/etc. Default `None` so
    /// platform::Window impls that don't need this (mac, wasm, test) don't have to implement it.
